...
export function PatientActions({
  ...
  return (
    <div className="surface-card p-5 space-y-4">
      <div>
        <h2 className="text-xs font-bold uppercase tracking-wider text-[var(--primary)]">Chart Actions</h2>
      </div>

      <div className="grid grid-cols-2 gap-2">
        <a className="btn-primary text-center px-3 py-2 text-sm" href={`/api/patients/${patientId}/export?format=pdf`}>
          PDF Export
        </a>
        <a className="btn-secondary text-center px-3 py-2 text-sm" href={`/api/patients/${patientId}/export?format=json`}>
          JSON Export
        </a>
        <a className="btn-secondary text-center px-3 py-2 text-sm" href="/api/calendar/me">
          Calendar ICS
        </a>
      ...
      {isAdmin && (
        <div className="border-t border-stone-200 pt-4 space-y-2">
          ...
          className="w-full rounded-lg border border-red-200 bg-[var(--surface)] px-3 py-2 text-sm"
          />
